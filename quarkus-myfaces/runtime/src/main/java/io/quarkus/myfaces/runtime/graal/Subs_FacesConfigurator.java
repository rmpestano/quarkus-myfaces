...
import org.apache.myfaces.config.*;
...
import org.apache.myfaces.flow.*;
...
import org.apache.myfaces.spi.*;
import org.apache.myfaces.util.*;
...
//@TargetClass(className = "org.apache.myfaces.config.FacesConfigurator")
//@Substitute
public final class Subs_FacesConfigurator {

    //@Substitute
    private void setFactories(String factoryName, Collection<String> factories, String defaultFactory) {
        Logger.getLogger(getClass().getName()).info("Graal FacesConfigurator!");
        Substitute_FactoryFinder.setFactory(factoryName, defaultFactory);
        for (String factory : factories) {
            if (!factory.equals(defaultFactory)) {
                Substitute_FactoryFinder.setFactory(factoryName, factory);
            }
        }
    }
    ...
    //@Substitute
    public Subs_FacesConfigurator(ExternalContext externalContext) {
        Logger.getLogger(getClass().getName()).info("Init DEFAULT FacesConfigurator");
    ...
    public void configure() throws FacesException {
        Logger.getLogger(getClass().getName()).info("Calling graal configure...");
    ...
    private void configureFactories() {
        Logger.getLogger(getClass().getName()).info("graal configureFactories...");
    ...
    private void configureApplication() {
        ...
        for (SystemEventListener systemEventListener : dispenser.getSystemEventListeners()) {
            ...
            try {
                ...
                javax.faces.event.SystemEventListener listener = (javax.faces.event.SystemEventListener) ClassUtils
                        .newInstance(systemEventListener.getSystemEventListenerClass());
        ...
        FlowHandlerFactory flowHandlerFactory = (FlowHandlerFactory) FactoryFinder
                .getFactory(FactoryFinder.FLOW_HANDLER_FACTORY);
    ...
    private void configureRuntimeConfig() {
        ...
        List<BeanEntry> injectedBeansAndMetaData = (List<BeanEntry>) _externalContext.getApplicationMap()
                .get(INJECTED_BEAN_STORAGE_KEY);
        ...
        String elResolverComparatorClass = MyfacesConfig.getCurrentInstance(_externalContext).getElResolverComparator();
        if (elResolverComparatorClass != null && !elResolverComparatorClass.isEmpty()) {
            try {
                Class<Comparator<ELResolver>> clazz = (Class<Comparator<ELResolver>>) ClassUtils
                        .classForName(elResolverComparatorClass);
        ...
        String elResolverPredicateClass = MyfacesConfig.getCurrentInstance(_externalContext).getElResolverPredicate();
    ...
    private void configureRenderKits() {
        ...
        for (String renderKitId : dispenser.getRenderKitIds()) {
            ...
            javax.faces.render.RenderKit renderKit = (javax.faces.render.RenderKit) ClassUtils
                    .buildApplicationObject(javax.faces.render.RenderKit.class, renderKitClass, null);
            ...
            for (ClientBehaviorRenderer clientBehaviorRenderer : clientBehaviorRenderers) {
                try {
                    javax.faces.render.ClientBehaviorRenderer behaviorRenderer = (javax.faces.render.ClientBehaviorRenderer) ClassUtils
                            .newInstance(clientBehaviorRenderer.getRendererClass());
